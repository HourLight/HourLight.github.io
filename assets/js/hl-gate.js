...
(function(){
  ...
  function createOverlay(){
    ...
    overlayEl.innerHTML = '<div style="text-align:center;max-width:420px;padding:48px 32px;animation:hlGF .5s ease">'
      ...
      + '<div style="font-size:13px;color:#a89878;line-height:2;margin-bottom:28px;letter-spacing:.05em">免費帳號即可使用所有覺察工具<br>用 Google 登入，一鍵就好</div>'
    ...
    document.getElementById('hlGG').onclick = function(){
      ...
      try {
        firebase.auth().signInWithPopup(new firebase.auth.GoogleAuthProvider()).then(function(cred){
          ...
          setTimeout(function(){b.textContent='用 Google 登入';},2500);
        });
