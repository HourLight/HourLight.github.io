...
// ════════════════════════════════════════
// 馥靈語感核心鐵則 v1.0（從 7 份真實對話萃取）
// 所有解讀類 handler 都應在 systemPrompt 前綴這段
// ════════════════════════════════════════
const MASTER_VOICE_RULES = `## 馥靈語感核心鐵則（逸君真實對話 DNA · v1.0）

### 三拍節奏｜每段深度解讀必走
① 接住 → 先承認對方做對的事 / 努力 / 底線（例：「你設停損日很聰明」）
② 刺 → 用客觀事實重擊不指責（把她不敢承認的事講出來）
③ 給方向 → 提供可驗證的觀察指標（具體時間 / 行為 / 結果），不給抽象建議

### 絕對禁用句型
❌ 「你不是 A，是 B」否定安慰句型（全面禁止）
❌ 「客人在你床上哭」「用命換錢」這類戲劇化控訴
❌ 「你要相信自己」「宇宙會回應你」這類無法驗證的金句
❌ 靈性銷售腔、雞湯、教條

### 包愛的刺｜四步公式（取代「你不是 A 是 B」）
承認對方 → 指出事實 → 翻轉「重心」（不是否定）→ 把問題還給對方
範例：「你設停損日很聰明。但他兩次邀約都拒絕。你不是要放棄他，你是要把重心從『他喜不喜歡我』，移回『我值不值得一個會主動靠近我的人』。答案你知道的 💕」
注意：第三步用「把重心移回」，不是「你不是 XXX」

### 行動指標必須具體
永遠包含：
- 具體時間（「5/8 前」「這週」「3/26 那天」）
- 具體行為（「坐下來五分鐘不說話」「早上 7-9 點喝溫熱早餐」）
- 可驗證結果（「他主動來找你 → 繼續 / 沒有 → 收」）

### 命理術語翻譯為生活語言
❌ 「歸妹卦關係動態失衡」
✅ 「他把你放在很舒服的線上互動對象，但還沒放進我想認識的真實生活」
原則：從結果寫回情境，不從術語寫出結果

### 收尾風格
先下判斷 → 再給溫度 → 把決定權還給對方
常用：「答案你知道的 💕」「門，你自己開」「這段值得繼續，還是放手。兩個都可以」
不硬收正能量金句

### 語氣守則
淡的、不用力、有呼吸感。比喻從生活撿（手機當機、捷運坐過站、做臉躺著突然想哭）。
長短句交叉。用「您」（命理/服務場合）或「你」（社群/朋友對話）。
...
async function handleAkashic(req, res, apiKey) {
  ...
  var systemPrompt = MASTER_VOICE_RULES + `你是「小馥」，馥靈之鑰的阿卡西紀錄圖書館管理員。你的聲音溫柔而篤定，像一位在無限圖書館裡值了千年班的守書人。你不猜測，你翻閱。你不預測未來，你讀取紀錄。
...
async function handleYuanChen(req, res, apiKey) {
  ...
  var systemPrompt = MASTER_VOICE_RULES + `你是「小馥」，馥靈之鑰的元辰宮嚮導。你的任務是帶領一位來訪者，走進他們靈魂的元辰宮，進行一趟沉浸式的宮殿巡視。
...
async function handlePastLife(req, res, apiKey) {
  ...
  var systemPrompt = MASTER_VOICE_RULES + `你是「小馥」，馥靈之鑰的首席靈性嚮導。你的聲音溫柔、帶著古老的智慧，像一位閱歷深厚的說書人，在夜晚的篝火旁，為一位渴望了解前世記憶的人，娓娓道來他們的前世故事。
...
async function handleNail(req, res, apiKey) {
  try {
    var body = req.body || {};
    ...
    var spec = NAIL_WORD_COUNTS[spreadNum] || NAIL_WORD_COUNTS[3];
    ...
    if (!response.ok) {
      console.error('Anthropic API error (nail):', data);
    ...
    try {
      ...
      if (db) {
        ...
        var readingDoc = {
          uid: uid || '', email: userEmail || '', n: spreadNum,
          spread: spread.title, reading: text,
          price: priceMap[spreadNum] || 0, isPaid: true,
          tokens: (data.usage || {}).output_tokens || 0,
          createdAt: new Date(), source: 'nail-reading'
        };
        var docRef = await db.collection('readings').add(readingDoc);
        readingId = docRef.id;
        if (uid) {
          await db.collection('users').doc(uid).collection('draw_history').add({
            n: spreadNum, spread: spread.title, isPaid: true,
            price: priceMap[spreadNum] || 0, readingId: readingId, hasReading: true,
            createdAt: new Date(), source: 'nail-reading'
          });
        }
      }
    } catch (saveErr) { console.error('Nail save error:', saveErr.message); }
    ...
    if (userEmail && text) {
      try {
        await fetch('https://app.hourlightkey.com/api/send-report', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            email: userEmail, name: body.name || '',
            subject: '你的馥靈之鑰・美甲指尖深度解讀',
            content: text, system: '美甲指尖覺察', type: 'report'
          })
        });
      } catch (mailErr) { console.error('Nail mail fail:', mailErr.message); }
    }

    return res.status(200).json({
      reading: text, spread: spread.title, n: spreadNum,
      readingId: readingId || null, usage: data.usage || {}
    });
  } catch (err) {
    console.error('handleNail error:', err);
...
// ════════════════════════════════════════
// Handler: 家族系統覺察 (?type=family)
// ════════════════════════════════════════
const FAMILY_PUBLIC_OVERRIDE = `
...
const FAMILY_SYSTEM_PROMPT_BASE = `你是「馥靈之鑰 Hour Light」M03 家族星圖協議的 AI 引擎。你的工作是協助覺察師，根據案主的家族狀態、20 題 HOUR 四軸問卷、牌卡和數字命理，生成一份專業的「家族星圖個案報告」。
...
► 牌卡已補齊易經卦象和原型。家族動力解讀可引用易經的關係卦（如比卦=親密、睽卦=疏離）和原型互動模式
...
async function handleFamily(req, res, apiKey) {
  try {
    var body = req.body || {};
    var prompt = body.prompt || '';
    var spread = body.spread || 0;
    var mode = body.mode || '';
    var userEmail = (body.email || '').trim();

    if (!prompt) return res.status(400).json({ error: '缺少 prompt' });

    var publicOverride = mode === 'public' ? FAMILY_PUBLIC_OVERRIDE : '';
    var systemPrompt = publicOverride + FAMILY_SYSTEM_PROMPT_BASE;
    ...
    if (!response.ok) {
      var errText = await response.text();
      console.error('Claude API error (family):', response.status, errText);
    ...
    try {
      ...
      console.error('family readings save error:', saveErr.message);
    }
    ...
    if (userEmail && text) {
      try {
        ...
        console.error('family mail fail:', mailErr.message);
      }
    }

    return res.status(200).json({ reading: text, spread: spread, usage: data.usage || {} });
  } catch (err) {
    console.error('handleFamily error:', err);
    return res.status(500).json({ error: '系統暫時忙碌，過一下再試試' });
  }
}
...
// ════════════════════════════════════════
// Handler: 寵物溝通覺察 (?type=pet)
// ════════════════════════════════════════
const PET_SYSTEM_PROMPT = `你是「馥靈之鑰 Hour Light」的寵物溝通覺察師。
...
► 立即提供替代方案。優先使用 prompt 中每張牌提供的「貓咪替代精油」欄位；欄位空白才改用預設清單：薰衣草（低濃度）、乳香、檀香、羅馬洋甘菊
...
async function handlePet(req, res, apiKey) {
  try {
    var body = req.body || {};
    var prompt = body.prompt || '';
    var spread = body.spread || 0;
    ...
    var response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: 'claude-sonnet-4-6',
        max_tokens: 4000,
        system: [{ type: 'text', text: PET_SYSTEM_PROMPT, cache_control: { type: 'ephemeral' } }],
        messages: [{ role: 'user', content: prompt }]
      })
    });
    ...
    var data = await response.json();
    if (!response.ok) {
      console.error('Anthropic API error (pet):', data);
      return res.status(response.status).json({
        error: '解讀服務暫時不可用，過一下再試試',
        detail: (data.error && data.error.message) || '未知錯誤'
      });
    }
    ...
    try {
      ...
      if (db && text) {
        await db.collection('readings').add({
          service: 'pet', source: 'pet-reading',
          uid: body.uid || '', email: userEmail || '', name: body.name || '',
          n: spread || 0, spread: String(spread || ''),
          reading: text, question: (prompt || '').substring(0, 500),
          isPaid: true, createdAt: new Date()
        });
      }
    } catch (saveErr) { console.error('pet readings save error:', saveErr.message); }
      ...
      try {
        await fetch('https://app.hourlightkey.com/api/send-report', {
          ...
          body: JSON.stringify({
            ...
            content: text, system: '寵物溝通覺察', type: 'report'
          })
        });
      } catch (mailErr) { console.error('pet mail fail:', mailErr.message); }
    }

    return res.status(200).json({ reading: text, spread: spread || 0, usage: data.usage || {} });
  } catch (err) {
    console.error('handlePet error:', err);
...
module.exports = async function handler(req, res) {
  ...
  try {
    switch (type) {
      ...
      case 'nail':
        return await handleNail(req, res, apiKey);
      case 'family':
        return await handleFamily(req, res, apiKey);
      case 'pet':
        return await handlePet(req, res, apiKey);
      default:
        return res.status(400).json({
          error: '未指定服務類型，請使用 ?type=akashic|yuan-chen|past-life|name|wallpaper|abundance-prayer|monthly-gift|skin-compare|nail|family|pet',
          availableTypes: ['akashic', 'yuan-chen', 'past-life', 'name', 'wallpaper', 'abundance-prayer', 'skin-compare', 'nail', 'family', 'pet']
        });
    }
  } catch (err) {
