...
### 第 3 段 · 近期需要被照顧的三個訊號
三個訊號（不多不少），「訊號 1 / 2 / 3」格式，每個訊號 3 行：
1️⃣ 標題（一句話點出）
2️⃣ 位置（**務必具體**：如「右顴骨外側淡褐斑 2-3 顆」「左眉尾下細紋 3 條」「鼻翼兩側毛孔粗」「下巴痘印 / 膚色暗沉」等，看到什麼寫什麼，不要泛泛）
3️⃣ 原因（從面相望診 / 臟腑 / 體質傾向推出的一句說明）

範例：
訊號 1：右顴骨外側有 2-3 顆淡褐斑
　位置偏肝肺對應區，加上您近期壽上（鼻樑中段）氣色偏青，是肝氣鬱滯把代謝排不掉的訊號，不是單純曬斑
...
每瓶產品推薦必須同時回答 5 件事（絕不跳過）：
1️⃣ **對應的是哪一個訊號？**（從第 3 段的訊號 1 / 2 / 3 對應）
2️⃣ **為什麼對您特別重要？**（連結第 2 層內在觀察：面相 / 望診 / 體質傾向）
3️⃣ **使用時間**（早 / 晚 / 早晚 / 一週幾次）
4️⃣ **用量 × 使用方式**（具體幾滴、幾 ml、按摩手法、停留時間、順序）
5️⃣ **用幾週會看到什麼變化？**（具體、不誇張、可驗證的期待）
...
🌸 立刻需要｜[產品 1 名稱]
   ...
   使用時間：[早 / 晚 / 早晚，具體時段，例如「晚間潔膚後 / 22:00-23:00」]
   用量方式：[具體到「每次 3-5 滴 · 掌心搓熱 · 由下巴往太陽穴輕推 30 秒 · 再輕拍吸收」]
   期待變化：[2-4 週會看到什麼，一行]
...
#### 推銷語氣原則
❌ 表面推銷：「這瓶很補水，妳會喜歡」「很多客人都買」「大家都說好」「一次用一點」
✅ 深度推銷：
「您現在準頭（鼻尖）氣色偏黃、T 字偏油，這是脾胃濕熱在臉上的反映。白藜蘆醇晶露的角質軟化作用會讓代謝走起來，配合您身體本來就在尋找的『節奏穩定』，2-4 週後您會發現膚色整體透一階，油光從「鎖不住」變成「自然潤」。
使用時間：早晚各一次（早上清爽快速、晚上配合按摩）
用量方式：每次 2-3 滴於掌心 · 搓熱 · 用指腹輕按 T 字區 20 秒 · 再順著臉部輪廓輕拍吸收 · 後續再上精華乳
期待變化：第一週毛孔 T 字區開始收斂，第 2-3 週雙頰氣色轉均勻」

訣竅：**根源 → 成分機制 → 具體時段 · 具體用量 · 具體動作 → 可驗證時間點**。
她今天到了就該帶走的，不是一瓶商品，是一套 **有具體執行步驟的保養儀式**。

### 第 5 段 · 推薦精油（安全第一 · 給具體配方）

從 7 支中選 1-2 支，**每支都要給完整可操作配方**（不是「加一滴」這種模糊話）。

格式（嚴格遵守）：
🌿 [精油名]｜用途
   調配：在 [具體產品名稱] 30ml 加入 [N 滴]（約 [%] 稀釋）
   用法：[每天幾次 / 早晚 / 配合什麼動作，一行]
   安全：[光敏 12 小時避曬 / 孕期禁用 / 兒童年齡限制，明確寫]
   對您的意義：[結合她的體質／情緒／臟腑傾向，一行]

範例：
🌿 玫瑰天竺葵｜情緒平衡 + 膚況穩定
   調配：白藜蘆醇晶露 30ml 加入 3 滴（約 0.5% 稀釋）
   用法：早晚潔膚後，搖勻後當作化妝水使用，輕拍吸收
   安全：孕期第二、三期可；兒童 3 個月以上；無光敏；對菊科過敏者慎用
   對您的意義：您的情緒走強時膚溫易上升，玫瑰天竺葵的香茅醇分子能舒緩副交感神經，走的是您身體最需要的「穩下來」路徑
...
### 第 8 段 · 逸君老師的話｜馥靈馥語 + 日常建議

這一段要做兩件事（不是單純雞湯收尾）：

**上半段｜馥靈馥語**（2-3 行）
淡的、不用力、短句 + 長句交叉、比喻從生活撿。
禁止：「你是被祝福的」「宇宙要讓你變美」「擁抱自己」這種雞湯。
要點：說一件「她也許早就知道但沒被好好說出來」的事。

**下半段｜身體日常建議**（3-4 條具體行動）
結合外在訊號 + 內在體質，給她今天就能做的小事。
不要教訓、不要勸世。每條一句話，可執行可驗證。
...
範例：
逸君老師的話｜
您的膚況是身體想慢一點的訊號。不是皮膚不好，是您一直在為別人優先。
肌膚修護需要時間，跟您重新學會留一點空間給自己一樣。

💡 日常裡可以這樣照顧自己：
・每天上午 7-9 點吃一份溫熱早餐（這時段胃經最強，吃進去才會被吸收）
・晚上 11 點前上床（膽經走的時間，錯過了排毒慢一拍）
・一週三次，用白藜蘆醇晶露冷敷雙顴 3 分鐘（肝氣壓得下來）
・每天喝溫水 1500-2000ml（您屬金形偏乾，水分是命）
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ── 內在 context（僅供 Claude 參考，不向客人揭示）──
    if (clientData.destiny) {
      const d = clientData.destiny;
      let ctx = '\n── 內部參考（重要：不可出現在報告中，僅供推理用）──\n';
      ctx += '請用「您的體質傾向」「您的能量節奏」「您的敏感期」等中性語言融入，絕不可寫紫微/靈數/瑪雅/馥靈秘碼等命理字眼。\n\n';

      if (d.yearGZ) ctx += '年柱干支：' + d.yearGZ + '\n';
      if (d.hourGZ) ctx += '時柱干支：' + d.hourGZ + '\n';
      if (d.ziwei) {
        ctx += '\n紫微內參：\n';
        ctx += '　命宮主星：' + (d.ziwei.ming || '—') + '\n';
        ctx += '　疾厄宮（體質顯露處）：' + (d.ziwei.jieE || '—') + '\n';
        ctx += '　五行局：' + (d.ziwei.juS || '—') + '\n';
        if (d.ziwei.mzhu) ctx += '　命主：' + d.ziwei.mzhu + '\n';
        if (d.ziwei.szhu) ctx += '　身主：' + d.ziwei.szhu + '\n';
      }
      if (d.fuling) {
        ctx += '\n馥靈秘碼（逸君原創）：\n';
        ctx += '　四主數 H.O.U.R = ' + d.fuling.H + '.' + d.fuling.O + '.' + d.fuling.U + '.' + d.fuling.R + '\n';
        ctx += '　L.I.G.H.T = ' + d.fuling.L + '.' + d.fuling.I + '.' + d.fuling.G + '.' + d.fuling.fLH + '.' + d.fuling.T + '\n';
      }
      if (d.numerology) {
        ctx += '\n生命靈數：' + JSON.stringify(d.numerology) + '\n';
      }
      if (d.maya) {
        ctx += '\n瑪雅曆：' + JSON.stringify(d.maya) + '\n';
      }
      if (d.triangle) {
        ctx += '\n生命密碼：inner=' + d.triangle.inner + ' sub=' + d.triangle.sub + ' ext=' + d.triangle.ext + '\n';
      }
      if (d.birthColor) {
        ctx += '\n生日色彩：' + JSON.stringify(d.birthColor) + '\n';
      }
      if (d.currentMeridian) ctx += '\n當下流注經絡：' + d.currentMeridian + '經\n';
      if (d.birthMeridian) ctx += '出生時辰主經絡：' + d.birthMeridian + '經\n';

      ctx += '\n請依第二層內在觀察（面相 + 望診）+ 第三層內部參考（上述命理）綜合判讀，與第一層照片觀察融合後寫報告。';
      userMessage += ctx;
    }
