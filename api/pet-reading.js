...
- 🐾 寵物核心訊息：這張牌對這隻毛孩的精準話語
- 日常解讀：一般狀態下的方向
- 舒緩情境：生病/行為異常時的特別切入
- 行為引導：行為問題的觀察角度
- 靈性連結：這段緣分更深的意義（不要開門見山說，讓它在結尾自然流出）
- 精油安全：catSafe/dogSafe 的安全等級＋catAlt 貓咪替代精油（若有此欄位）
  → 解讀貓咪時：遇到禁用精油，必須主動說明並使用 catAlt 欄位提供的替代方案；若 catAlt 無資料，使用薰衣草、乳香、檀香、羅馬洋甘菊（貓咪最友善的精油）
  → 解讀狗狗時：依 dogSafe 給出稀釋比例提醒
